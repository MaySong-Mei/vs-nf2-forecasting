...
>
> **Status:** public subset preprocessing, QC, frozen-cohort baselines, and formal five-fold CUDA training completed; the neural model showed systematic segmentation collapse and did not outperform either baseline
>
...
The original author scripts remain at this experiment directory root. The maintained public
reproduction lives in `ucsd_repro/`, `scripts/`, `configs/`, and `tests/`.

## Working directory after repository reorganization

Run the commands below from `reproductions/deepgrowth_ucsd/`, not the repository root. Existing code, configs, data, manifests and outputs moved together on 2026-09-11. The existing shared virtual environment remains at the repository root; activate it before changing directory. If creating a new per-experiment environment, the commands below remain valid.

Historical frozen artifacts can contain the previous absolute root path. Preserve those records and their hashes; validate paths and create new run metadata for reruns instead of silently rewriting historical evidence.
...
python scripts/aggregate_folds.py \
  --run-root outputs/checkpoints/ucsd_deepgrowth_public_reproduction_formal_103_73 \
  --output outputs/five_fold_summary_formal_103_73.json
```

## Formal five-fold result

The formal run completed on 2026-08-08 using the frozen 103-triplet / 73-patient
cohort, patient-level five-fold splits, 200 epochs per fold, and seeds 100–104.
All 103 held-out predictions produced finite metrics. Training and aggregation
took approximately 2 h 21 min on the audited RTX 3080 Laptop GPU. The local,
Git-ignored aggregate is
`outputs/five_fold_summary_formal_103_73.json`.

The result is a valid engineering run but **not a successful performance
reproduction**. The neural model underperformed both predeclared baselines by a
large margin:

| Method | Case-level Dice ↑ | HD95 (mm) ↓ | Absolute RVD ↓ | Volume absolute error (mm³) ↓ |
| --- | ---: | ---: | ---: | ---: |
| Stable-mask baseline | 0.5964 | 2.3616 | 0.3015 | 147.2 |
| Linear-SDF baseline | 0.4578 | 4.1685 | 3.0378 | 576.9 |
| Formal neural model | **0.0288** | **20.8602** | **328.4444** | **26,531.1** |

Performance was worse on the 21 cases in the top 20% of absolute target-volume
change: Dice 0.0116, HD95 22.4264 mm, absolute RVD 777.3056, and volume absolute
error 27,181.0 mm³. The five fold-level mean Dice scores ranged only from
0.0264 to 0.0305, so the failure was systematic rather than isolated to one
split.

Training histories identify a collapse mode. Fold-best checkpoints occurred at
epochs 21, 15, 11, 6, and 2 even though training loss continued to decrease.
By epoch 200, validation Dice was zero in every fold. At the selected early
checkpoints, mean predicted tumor volume was 27,374.8 mm³ versus 843.8 mm³ for
the target: predictions occupied 53.5% of the 64³ crop on average, while target
tumors occupied 1.65%.

The leading explanation is class/geometry imbalance in the current objective:
training uniformly samples points from the full SDF volume and applies
unweighted L1 losses. Tumor and near-surface points are therefore a small
minority. As optimization progresses, the network is rewarded for predicting
outside-tumor SDF values; late predictions become empty, while the slightly
higher-Dice early checkpoints retain near-random, severely oversegmented fields.
This diagnosis should be tested with tumor/near-surface stratified sampling and
a balanced SDF or segmentation-aware loss in a short single-fold ablation before
another full five-fold run.
...
The subsequent formal five-fold run completed on the same audited device. Peak
reserved CUDA memory was approximately 0.14 GiB per fold, and the five selected
checkpoints were evaluated and aggregated successfully. The poor metrics above
reflect model behavior rather than an interrupted run, missing fold, non-finite
metric, or CUDA failure.
